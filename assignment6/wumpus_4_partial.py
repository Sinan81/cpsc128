...
class Cave_System:
    ...
    def __init__(self, tunnels, system_map):
        self.rooms = []
        for i in range(len(tunnels)):
            self.rooms.append( Room(i, tunnels[i]) )
...
class Room:
    '''e.g. r = Room(0,[1,7,4])'''
    def __init__(self, room_number, connections, \
                 b = None, p = None, w = None):
...
class Player:
    ...
    def move(self, new_location):
        if new_location in cave_system.rooms[self.location].tunnels:
            self.location = new_location
        else:
            print("You can't walk through walls in this game!")
            
    def shoot(self):
        roomlist = input('What rooms [a, b, c]?')
        self.arrows = self.arrows -1
        # You will be filling this method in!
        print("Shooting ...")
        # uncomment the next line when you're ready to add the code it requires.
        # The return below is a temporary statement to allow the program to execute
...
# below we're using multiple assignment trick
...
# Initialize Player.
room_number = random.randint(0, len(cave_system.rooms) - 1)
player = Player(room_number, 5)
